if (typeof browser == "undefined") {
  // Chrome does not support the browser namespace, way to go Chrome
...
browser.runtime.onInstalled.addListener(() => {
  browser.contextMenus.create({
    ...
    contexts: ["link"],
    documentUrlPatterns: ["*://community.octoprint.org/*", "*://github.com/*"],
  });
  browser.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === "open-in-bundle-viewer") {
      browser.tabs
        ...
        .then((currentTab) => {
          const url =
            "https://bundleviewer.octoprint.org/?url=" +
